...
const basketQty = document.querySelector(".basket-qty");
...
const showCartItems = () => {
  ...
  const basketQuantity = () => {
    if (cartItems.length === 0) {
      ...
      basketQty.innerHTML = `${cartItems.length}`;
    }
  };
      ...
      ${cartItems.reduce((total, item) => {
        total += `
          <div class="cart-card">
            ...
            <div><button class="remove-cart-item-btn" data-id="${
              item.id
            }">Remove</button></div>
          </div>
          `;
  ...
  <p class="subtotal"><span style="color: black">Subtotal:</span> $${cartItems
